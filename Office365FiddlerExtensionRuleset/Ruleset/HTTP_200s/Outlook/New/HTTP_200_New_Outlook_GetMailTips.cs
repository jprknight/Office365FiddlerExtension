...
{
    class HTTP_200_New_Outlook_GetMailTips
    {
        internal Session session { get; set; }

        private static HTTP_200_New_Outlook_GetMailTips _instance;

        public static HTTP_200_New_Outlook_GetMailTips Instance => _instance ?? (_instance = new HTTP_200_New_Outlook_GetMailTips());

        public void Run(Session session)
        {
            this.session = session;
            
            if (!this.session.uriContains("owa/service.svc?action=GetMailTips"))
            {
                return;
            }

            FiddlerApplication.Log.LogString($"{Assembly.GetExecutingAssembly().GetName().Name} ({this.GetType().Name}): " + this.session.id + " HTTP 200 New Outlook GetMailTips.");

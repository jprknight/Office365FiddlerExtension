...
namespace Office365FiddlerInspector.Ruleset
{
    class HTTP_200 : ActivationService
    {
        ...
        public void HTTP_200_Outlook_Mapi(Session session)
        {
            ...
            if (session.HostnameIs("outlook.office365.com") && (session.uriContains("/mapi/emsmdb/?MailboxId=")))
            {
                ...
                else
                {
                    ...
                    session["X-SessionType"] = "Outlook M365 MAPI";

                    session["X-ResponseAlert"] = "Outlook for Windows M365 MAPI traffic";
                    session["X-ResponseComments"] = "This is normal Outlook MAPI over HTTP traffic to an Exchange Online / M365 mailbox.";

                    session["X-ResponseCodeDescription"] = "200 OK";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtention: " + session.id + " HTTP 200 Outlook M365 MAPI traffic.");
            ...
            else
            {
                ...
                session["X-ResponseComments"] = "This is normal Outlook MAPI over HTTP traffic to an Exchange mailbox.";
        ...
        public void HTTP_200_Outlook_NSPI(Session session)
        {
            ...
            //
            if (session.uriContains("/mapi/nspi/"))
            {
                session["ui-backcolor"] = Preferences.HTMLColourGreen;
                session["ui-color"] = "black";

                session["X-SessionType"] = "Outlook NSPI";

                session["X-ResponseAlert"] = "Outlook for Windows NSPI traffic";
                session["X-ResponseComments"] = "This is normal Outlook traffic to an Exchange On-Premise mailbox. Name Service Provider Interface (NSPI).";

                // No FiddlerApplication logging here.

                session["X-ResponseCodeDescription"] = "200 OK";

                FiddlerApplication.Log.LogString("Office365FiddlerExtention: " + session.id + " HTTP 200 Outlook NSPI traffic.");
        ...
        public void HTTP_200_OnPremise_AutoDiscover_Redirect(Session session)
        {
            ...
            if (session.utilFindInResponse("<Action>redirectAddr</Action>", false) > 1)
            {
                if (!session.HostnameIs("autodiscover-s.outlook.com"))
                {
                    /*
                    ...
                    */
                    ...
                    // 
                    string RedirectResponseBody = session.GetResponseBodyAsString();
                    int start = session.GetResponseBodyAsString().IndexOf("<RedirectAddr>");
                    int end = session.GetResponseBodyAsString().IndexOf("</RedirectAddr>");
                    ...
                    if (RedirectAddress.Contains(".onmicrosoft.com"))
                    {
                        ...
                        session["X-SessionType"] = "On-Prem AutoD Redirect";

                        session["X-ResponseAlert"] = "Exchange On-Premise Autodiscover redirect.";
                        session["X-ResponseComments"] = "Exchange On-Premise Autodiscover redirect address to Exchange Online found."
                            + "<p>RedirectAddress: "
                            + RedirectAddress
                    ...
                    else
                    {
                        ...
                        session["X-SessionType"] = "!AUTOD REDIRECT ADDR!";

                        session["X-ResponseAlert"] = "!Exchange On-Premise Autodiscover redirect!";
                        session["X-ResponseComments"] = "Exchange On-Premise Autodiscover redirect address found, which does not contain .onmicrosoft.com." +
                        ...
                        FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 Exchange On-Premise AUTOD REDIRECT ADDR! : " + RedirectAddress);

                        session["X-ResponseCodeDescription"] = "200 OK, Incorrect Redirect Address!";
        ...
        public void HTTP_200_OnPremise_AutoDiscover_Redirect_AddressNotFound(Session session)
        {
            ...
            //
            if ((session.utilFindInResponse("<Message>The email address can't be found.</Message>", false) > 1) &&
                (session.utilFindInResponse("<ErrorCode>500</ErrorCode>", false) > 1))
            {
                /*
                <?xml version="1.0" encoding="utf-8"?>
                <Autodiscover xmlns="http://schemas.microsoft.com/exchange/autodiscover/responseschema/2006">
                ...
                */
                session["ui-backcolor"] = Preferences.HTMLColourRed;
                session["ui-color"] = "black";
                session["X-SessionType"] = "!NO AUTOD REDIRECT ADDR!";

                session["X-ResponseAlert"] = "<b><span style='color:red'>Exchange On-Premise Autodiscover Redirect</span></b>";
                session["X-ResponseComments"] = "Exchange On-Premise Autodiscover redirect address can't be found. "
                ...
                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 Exchange On-Premise redirect address. Error code 500: The email address can't be found.");

                session["X-ResponseCodeDescription"] = "200 OK, Email address not found.";
        ...
        public void HTTP_200_EXO_M365_AutoDiscover(Session session)
        {
            ...
            if ((session.hostname == "autodiscover-s.outlook.com") && (session.uriContains("autodiscover.xml")))
            {
                if ((session.utilFindInResponse("<DisplayName>", false) > 1) &&
                    (session.utilFindInResponse("<MicrosoftOnline>", false) > 1) &&
                    (session.utilFindInResponse("<MailStore>", false) > 1) &&
                    (session.utilFindInResponse("<ExternalUrl>", false) > 1))
                {
                    session["ui-backcolor"] = Preferences.HTMLColourGreen;
                    session["ui-color"] = "black";
                    session["X-SessionType"] = "EXO MSI Autodiscover";

                    session["X-ResponseAlert"] = "Exchange Online / Outlook MSI Autodiscover.";
                    session["X-ResponseComments"] = "For Autodiscover calls which go to autodiscover-s.outlook.com this is likely an Outlook (MSI / perpetual license) client"
                        + " being redirected from Exchange On-Premise to Exchange Online.";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 Exchange Online / Outlook MSI Autodiscover. Expected XML found.");
                ...
                else
                {
                    ...
                    session["X-ResponseAlert"] = "<b><span style='color:red'>Exchange Online / Outlook MSI Autodiscover - Unusual Autodiscover Response</span></b>";
                    session["X-ResponseComments"] = "This session was detected as an Autodiscover response from Exchange Online. However the response did not contain the expected XML data."
                    ...
                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 Exchange Online / Outlook MSI Autodiscover. Expected XML NOT found!");

                    session["X-ResponseCodeDescription"] = "200 OK, Unexpected AutoDiscover XML response.";
            ...
            else if ((session.hostname == "outlook.office365.com") && (session.uriContains("autodiscover.xml")))
            {
                if ((session.utilFindInResponse("<DisplayName>", false) > 1) &&
                    (session.utilFindInResponse("<MicrosoftOnline>", false) > 1) &&
                    (session.utilFindInResponse("<MailStore>", false) > 1) &&
                    (session.utilFindInResponse("<ExternalUrl>", false) > 1))
                {
                    ...
                    session["X-SessionType"] = "EXO CTR Autodiscover";

                    session["X-ResponseAlert"] = "Exchange Online / Outlook CTR Autodiscover.";
                    session["X-ResponseComments"] = "For Autodiscover calls which go to outlook.office365.com this is likely an Outlook Click-To-Run (Downloaded or "
                        + "deployed from Office365) client being redirected from Exchange On-Premise to Exchange Online.";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 Exchange Online / Outlook CTR Autodiscover. Expected XML found.");
                ...
                else
                {
                    ...
                    session["X-ResponseAlert"] = "<b><span style='color:red'>Exchange Online / Outlook CTR Autodiscover - Unusual Autodiscover Response</span></b>";
                    session["X-ResponseComments"] = "This session was detected as an Autodiscover response from Exchange Online. However the response did not contain the expected XML data."
                        + " Check if a device in-between the perimeter of your network and the client computer can / has altered the data in the response.";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 Exchange Online / Outlook CTR Autodiscover. Expected XML NOT found!");

                    session["X-ResponseCodeDescription"] = "200 OK, Unexpected XML response.";
        ...
        public void HTTP_200_Unified_Groups_Settings(Session session)
        {
            ...
            if (session.HostnameIs("outlook.office365.com") &&
                (session.uriContains("ews/exchange.asmx") &&
                (session.utilFindInRequest("GetUnifiedGroupsSettings", false) > 1)))
            {
                // User can create Office 365 gropus.
                if (session.utilFindInResponse("<GroupCreationEnabled>true</GroupCreationEnabled>", false) > 1)
                {
                    session["ui-backcolor"] = Preferences.HTMLColourGreen;
                    session["ui-color"] = "black";
                    session["X-SessionType"] = "EWS GetUnifiedGroupsSettings";

                    session["X-ResponseAlert"] = "GetUnifiedGroupsSettings EWS call.";
                    session["X-ResponseComments"] = "<GroupCreationEnabled>true</GroupCreationEnabled> found in response body. "
                        + "Expect user to be able to create Office 365 groups in Outlook.";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 GetUnifiedGroupsSettings EWS call. User can create O365 Groups in Outlook.");
                ...
                else if (session.utilFindInResponse("<GroupCreationEnabled>false</GroupCreationEnabled>", false) > 1)
                {
                    session["ui-backcolor"] = Preferences.HTMLColourGreen;
                    session["ui-color"] = "black";
                    ...
                    session["X-ResponseAlert"] = "<b><span style='color:red'>GetUnifiedGroupsSettings EWS call</span></b>";
                    session["X-ResponseComments"] = "<GroupCreationEnabled>false</GroupCreationEnabled> found in response body. "
                        + "Expect user to <b>NOT be able to create Office 365 groups</b> in Outlook.";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 GetUnifiedGroupsSettings EWS call. User cannot create O365 Groups in Outlook.");

                    session["X-ResponseCodeDescription"] = "200 OK, User cannot create Unified Groups.";
                ...
                else
                {
                    ...
                    session["X-ResponseAlert"] = "GetUnifiedGroupsSettings EWS call";
                    session["X-ResponseComments"] = "Though GetUnifiedGroupsSettings scenario was detected neither <GroupCreationEnabled>true</GroupCreationEnabled> or"
                        + "<GroupCreationEnabled>false</GroupCreationEnabled> was found in the response body. Check the Raw tab for more details.";

                    FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 GetUnifiedGroupsSettings!");

                    session["X-ResponseCodeDescription"] = "200 OK, GetUnifiedGroupsSettings not found.";
        ...
        public void HTTP_200_3S_Suggestions(Session session)
        {
            ...
            //
            if (session.uriContains("search/api/v1/suggestions"))
            {
                session["ui-backcolor"] = Preferences.HTMLColourGreen;
                session["ui-color"] = "black";
                session["X-SessionType"] = "3S Suggestions";
                ...
                var clientRequestId = session.RequestHeaders.Where(x => x.Name.Equals("client-request-id")).FirstOrDefault();

                session["X-ResponseAlert"] = "3S Suggestions";
                session["X-ResponseComments"] = $"Scenario: {scenario} Types: {entityTypes} {clientRequestId}";

                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " 200 3S Suggestions call.");
        ...
        public void HTTP_200_REST_People_Request(Session session)
        {
            ...
            if (session.uriContains("people"))
            {
                ...
                if (session.uriContains("/me/people"))
                {
                    sessionType = "Private";
                }

                // /users()/people : Public FindPeople Request
                else if (session.uriContains("/users(") && session.uriContains("/people"))
                {
                    sessionType = "Public";
                }

                var requestId = session.ResponseHeaders.Where(x => x.Name.Equals("request-id")).FirstOrDefault();

                session["X-SessionType"] = $"REST People {sessionType}";
                session["X-ResponseAlert"] = $"REST People {sessionType}";
                session["X-ResponseComments"] = $"{requestId} $search:{queryStrings["$search"]} $top:{queryStrings["$top"]} $skip:{queryStrings["$skip"]} $select:{queryStrings["$select"]} $filter:{queryStrings["$filter"]}";

                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " 200 REST - People Request.");
        ...
        public void HTTP_200_Any_Other_Exchange_EWS(Session session)
        {
            ...
            if (session.HostnameIs("outlook.office365.com") &&
                        (session.uriContains("ews/exchange.asmx")))
            {
                session["X-SessionType"] = "Exchange Online / M365 Web Services";

                session["X-ResponseAlert"] = "Exchange Online / M365 Web Services (EWS) call.";
                session["X-ResponseComments"] = "Exchange Online / M365 Web Services (EWS) call.";

                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 EXO / M365 EWS call.");
            } 
            else if (session.uriContains("ews/exchange.asmx"))
            {
                session["X-SessionType"] = "Exchange OnPremise Web Services";

                session["X-ResponseAlert"] = "Exchange OnPremise Web Services (EWS) call.";
                session["X-ResponseComments"] = "Exchange OnPremise Web Services (EWS) call.";

                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 OnPremise EWS call.");
            }
        ...
        public void HTTP_200_Lurking_Errors(Session session)
        {
            ...
            string text200 = session.ToString();
            ...
            if (wordCountError > 0 || wordCountFailed > 0 || wordCountException > 0)
            {
                ...
                session["ui-backcolor"] = "black";
                session["ui-color"] = "red";
                session["X-SessionType"] = "!FAILURE LURKING!";

                session["X-ResponseAlert"] = "<b><span style='color:red'>'error', 'failed' or 'exception' found in response body</span></b>";
                session["X-ResponseComments"] += "<p>Session response body was scanned and errors or failures were found in response body. "
                ...
                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 200 FAILURE LURKING!?");

                session["X-ResponseCodeDescription"] = "200 OK, but possibly bad.";

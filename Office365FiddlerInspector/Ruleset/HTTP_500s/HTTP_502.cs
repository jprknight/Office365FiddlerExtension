...
namespace Office365FiddlerInspector.Ruleset
{
    class HTTP_502 : ActivationService
    {
        ...
        public void HTTP_502_Bad_Gateway_Telemetry_False_Positive(Session session)
        {
            ...
            //
            if ((session.oRequest["Host"] == "sqm.telemetry.microsoft.com:443") &&
                (session.utilFindInResponse("target machine actively refused it", false) > 1))
            {
                session["ui-backcolor"] = Preferences.HTMLColourBlue;
                session["ui-color"] = "black";

                session["X-SessionType"] = "False Positive";

                session["X-ResponseCodeDescription"] = "502 Bad Gateway False Positive";

                session["X-ResponseAlert"] = "<b><span style='color:green'>False Positive</span></b>";
                session["X-ResponseComments"] = "Telemetry failing is unlikely the cause of significant Office 365 client issues.";

                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 502 Bad Gateway. Telemetry False Positive.");
        ...
        public void HTTP_502_Bad_Gateway_EXO_DNS_Lookup_False_Positive(Session session)
        {
            ...
            if ((session.utilFindInResponse(".onmicrosoft.com", false) > 1) &&
                            (session.utilFindInResponse("DNS Lookup for ", false) > 1) &&
                            (session.utilFindInResponse(" failed.", false) > 1))
            {
                session["ui-backcolor"] = Preferences.HTMLColourBlue;
                session["ui-color"] = "black";

                session["X-Authentication"] = "False Positive";
                session["X-SessionType"] = "False Positive";
                session["X-ResponseServer"] = "False Positive";

                session["X-ResponseCodeDescription"] = "502 Bad Gateway False Positive";

                session["X-ResponseAlert"] = "<b><span style='color:green'>False Positive</span></b>";
                session["X-ResponseComments"] = "<b><span style='color:green'>False positive on HTTP 502</span></b>. "
                ...
                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 502 Bad Gateway. EXO DNS False Positive.");
        ...
        public void HTTP_502_Bad_Gateway_EXO_AutoDiscover_False_Positive(Session session)
        {
                        ...
                        (session.utilFindInResponse("target machine actively refused it", false) > 1))
            {
                ...
                string AutoDFalsePositiveResponseBody = session.GetResponseBodyAsString();
                int start = session.GetResponseBodyAsString().IndexOf("'");
                int end = session.GetResponseBodyAsString().LastIndexOf("'");
                ...
                session["X-ResponseAlert"] = "<b><span style='color:green'>False Positive</span></b>";
                session["X-ResponseComments"] = "<b><span style='color:green'>False Positive</span></b>. By design Office 365 Autodiscover does not respond to "
                ...
                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 502 Bad Gateway. O365 AutoD onmicrosoft.com False Positive.");
        ...
        public void HTTP_502_Bad_Gateway_Vanity_Domain_M365_AutoDiscover_False_Positive(Session session)
        {
            ...
            //
                ...
                */
                ...
                session["X-Authentication"] = "AutoD False Positive";
                session["X-SessionType"] = "AutoD False Positive";
                session["X-ResponseServer"] = "AutoD False Positive";

                session["X-ResponseCodeDescription"] = "502 Bad Gateway False Positive";

                session["X-ResponseAlert"] = "<b><span style='color:orange'>Autodiscover Possible False Positive?</span></b>";
                session["X-ResponseComments"] = "Autoddiscover Possible False Positive. By design Office 365 endpoints such as autodiscover.contoso.onmicrosoft.com "
                ...
                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 502 Bad Gateway. Vanity domain AutoD False Positive.");
        ...
        public void HTTP_502_Bad_Gateway_Anything_Else_AutoDiscover(Session session)
        {
            ...
            if ((session.utilFindInResponse("target machine actively refused it", false) > 1) &&
                        (session.utilFindInResponse("autodiscover", false) > 1))
            {
                session["ui-backcolor"] = Preferences.HTMLColourRed;
                session["ui-color"] = "black";
                session["X-SessionType"] = "!AUTODISCOVER!";

                session["X-ResponseCodeDescription"] = "502 Bad Gateway";

                session["X-ResponseAlert"] = "<b><span style='color:red'>AUTODISCOVER</span></b>";
                session["X-ResponseComments"] = "This AutoDiscover request was refused by the server it was sent to. Check the raw tab for further details.";

                FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 502 Bad Gateway. Exchange Autodiscover.");
        ...
        public void HTTP_502_Bad_Gateway_Anything_Else(Session session)
        {
            ...
            session["ui-backcolor"] = Preferences.HTMLColourRed;
            session["ui-color"] = "black";
            session["X-SessionType"] = "!Bad Gateway!";

            session["X-ResponseCodeDescription"] = "502 Bad Gateway";

            session["X-ResponseAlert"] = "<b><span style='color:red'>HTTP 502 Bad Gateway</span></b>";
            session["X-ResponseComments"] = "Potential to cause the issue you are investigating. "
            ...
            FiddlerApplication.Log.LogString("Office365FiddlerExtension: " + session.id + " HTTP 502 Bad Gateway (99).");

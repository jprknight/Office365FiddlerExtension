...
Earlier versions of the extension were deployed with a .net installer. It is best to uninstall these before installing the newer version with the deployment script. However, it isn't necessary, the deployment script will remove old versions of the extension dll when running the install routine.
...
1. **Reproduce an issue** from the computer where it occurs:
...
* Opening or double clicking the **SAZ file**. As Fiddler loads the SAZ file the sessions are automatically analysed.
...
Think of traffic lights, with some extras.
...
* Blue -- False positive detected, most prominently HTTP 502's, see Session Analysis on the response inspector for details.
...
* **Severity** - Session Severity has a scale of 0 – 60. The scale of Session Severity correlates with the colourisation of sessions the extension provides:
...
Assuming you don't have never web call enabled, the extension pulls from the Microsoft URLs and IPs Web Service, to tell you if a host IP is:
  * A private LAN IP address. Host IP will show like "LAN:10.0.0.1".
  * A public IP address. Host IP will show like "PUB:8.8.8.8".
  * A Microsoft 365 IP address. Host IP will show like "M365:40.99.10.34".
* **Authentication** - Authentication details detected in the session.

### Other Information

- In comparison to earlier versions of the extension the structure of the **solution is now much more comparmentalised**. Individual classes can be updated for specific purposes without impacting on the whole solution.
